...
const STEPS = ['menu', 'cart', 'details', 'confirm'];
...
export default function Home() {
  ...
  const updateQty = (key, delta) => {
    setCart(cart.map(c => c.key === key ? { ...c, qty: Math.max(1, c.qty + delta) } : c).filter(c => c.qty > 0));
  };
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Order freshly baked cakes, cupcakes and pastries from Blissful Bakes" />
      ...
      <div style={{ minHeight: '100vh', background: 'var(--cream)' }}>
        ...
        <header style={{
          ...
          <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64 }}>
            <div>
              ...
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)', letterSpacing: 2, textTransform: 'uppercase' }}>Freshly Baked Edition</div>
            </div>
            {step === 'menu' && (
              <button onClick={() => cart.length && setStep('cart')} style={{
                ...
                🛒 Cart {cartCount > 0 && <span style={{ background: 'var(--brown)', borderRadius: '50%', width: 22, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12 }}>{cartCount}</span>}
              </button>
            )}
          </div>
        </header>
        ...
        <main style={{ maxWidth: 1100, margin: '0 auto', padding: '0 24px 60px' }}>
          ...
          {step === 'menu' && (
            <div style={{ animation: 'fadeIn 0.4s ease' }}>
              ...
              <div style={{ textAlign: 'center', padding: '48px 0 32px' }}>
                ...
                <h1 style={{ fontSize: 'clamp(28px,5vw,48px)', color: 'var(--brown)', lineHeight: 1.2 }}>
                  Freshly Baked,<br />
                  <span style={{ color: 'var(--gold)', fontStyle: 'italic' }}>Made with Love</span>
                </h1>
                <p style={{ color: 'var(--text-muted)', marginTop: 12, fontSize: 16 }}>
                  Choose from our freshly baked cakes and cupcakes
                </p>
              </div>
                ...
                {filtered.map(item => {
                  ...
                  return (
                    <div key={item.id} style={{
                      ...
                      onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.boxShadow = '0 8px 24px rgba(61,35,20,0.15)'; }}
                      onMouseLeave={e => { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = '0 2px 12px rgba(61,35,20,0.08)'; }}
                    >
                      <div style={{ background: 'linear-gradient(135deg, var(--cream), #f5e6d3)', height: 120, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 56, position: 'relative' }}>
                        {item.emoji}
                        {inCart > 0 && (
                          <div style={{ position: 'absolute', top: 10, right: 10, background: 'var(--gold)', color: 'white', borderRadius: '50%', width: 24, height: 24, fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{inCart}</div>
                        )}
                      </div>
                      <div style={{ padding: 16 }}>
                        <h3 style={{ fontSize: 18, color: 'var(--brown)', marginBottom: 4 }}>{item.name}</h3>
              ...
              {cartCount > 0 && (
                <div style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', zIndex: 200, animation: 'slideUp 0.3s ease' }}>
                  <button onClick={() => setStep('cart')} style={{
                    ...
                    fontSize: 16,
                    fontWeight: 700,
                    boxShadow: '0 8px 32px rgba(61,35,20,0.4)',
                    display: 'flex',
                    alignItems: 'center',
                    gap: 12,
                    whiteSpace: 'nowrap',
                    ...
                    <span style={{ background: 'var(--gold)', borderRadius: 30, padding: '3px 12px', fontSize: 14 }}>Ksh {total.toLocaleString()}</span>
                  </button>
                </div>
              )}
            </div>
          ...
          {/* CART STEP */}
          {step === 'cart' && (
            <div style={{ animation: 'fadeIn 0.4s ease', maxWidth: 620, margin: '0 auto' }}>
              <div style={{ paddingTop: 40 }}>
                <button onClick={() => setStep('menu')} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', fontSize: 14, marginBottom: 20, display: 'flex', alignItems: 'center', gap: 6 }}>
                  ← Back to Menu
                </button>
                ...
                {cart.length === 0 ? (
                  ...
                  <>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 24 }}>
                      {cart.map(c => (
                        <div key={c.key} style={{ background: 'white', borderRadius: 14, padding: 16, border: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 14 }}>
                          <div style={{ fontSize: 32 }}>{c.item.emoji}</div>
                          <div style={{ flex: 1 }}>
                            <div style={{ fontWeight: 600, color: 'var(--brown)' }}>{c.item.name}</div>
                            <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>{c.size.label}</div>
                          </div>
                          ...
                          <button onClick={() => removeFromCart(c.key)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', fontSize: 18, cursor: 'pointer', padding: '0 4px' }}>×</button>
                        </div>
                      ))}
                    </div>
                    ...
                    <button onClick={() => setStep('details')} style={{ width: '100%', background: 'var(--gold)', color: 'var(--brown)', border: 'none', borderRadius: 12, padding: '16px', fontSize: 16, fontWeight: 700, cursor: 'pointer' }}>
                      Proceed to Checkout →
                    </button>
                  </>
                )}
          ...
          {step === 'details' && (
            <div style={{ animation: 'fadeIn 0.4s ease', maxWidth: 560, margin: '0 auto' }}>
              <div style={{ paddingTop: 40 }}>
                <button onClick={() => setStep('cart')} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', fontSize: 14, marginBottom: 20, display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
                  ← Back to Cart
                </button>
                <h2 style={{ fontSize: 28, color: 'var(--brown)', marginBottom: 6 }}>Your Details</h2>
                <p style={{ color: 'var(--text-muted)', marginBottom: 28 }}>So we know where to deliver your order 🎂</p>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  <div>
                    ...
                    <textarea
                      placeholder="e.g. custom message on cake, allergies, preferred delivery time..."
                      value={customerInfo.notes}
                      onChange={e => setCustomerInfo(i => ({ ...i, notes: e.target.value }))}
                      rows={3}
                      style={{ width: '100%', padding: '12px 16px', borderRadius: 10, border: '1.5px solid var(--border)', background: 'white', fontSize: 15, outline: 'none', resize: 'vertical', transition: 'border-color 0.2s' }}
                      onFocus={e => e.target.style.borderColor = 'var(--brown)'}
                      onBlur={e => e.target.style.borderColor = 'var(--border)'}
                    />
                  </div>
                ...
                <div style={{ background: 'white', borderRadius: 14, border: '1px solid var(--border)', padding: 16, marginTop: 24, marginBottom: 20 }}>
                  <div style={{ fontWeight: 700, color: 'var(--brown)', marginBottom: 10, fontSize: 14, textTransform: 'uppercase', letterSpacing: 1 }}>Order Summary</div>
                  {cart.map(c => (
                ...
                <button
                  ...
                  style={{
                    width: '100%',
                    background: (!customerInfo.name || !customerInfo.phone || !customerInfo.address) ? 'var(--border)' : 'var(--brown)',
                    color: (!customerInfo.name || !customerInfo.phone || !customerInfo.address) ? 'var(--text-muted)' : 'white',
                    border: 'none',
                    borderRadius: 12,
                    padding: '16px',
                    fontSize: 16,
                    fontWeight: 700,
                    cursor: (!customerInfo.name || !customerInfo.phone || !customerInfo.address) ? 'not-allowed' : 'pointer',
                    transition: 'all 0.2s',
                  }}
                >
          ...
          {step === 'confirm' && placedOrder && (
            <div style={{ animation: 'fadeIn 0.5s ease', maxWidth: 560, margin: '0 auto', textAlign: 'center' }}>
              <div style={{ paddingTop: 60 }}>
                ...
                <div style={{ background: 'white', borderRadius: 16, border: '1px solid var(--border)', padding: '20px 24px', textAlign: 'left', marginBottom: 28 }}>
                  ...
                  <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--border)', fontSize: 13, color: 'var(--text-muted)' }}>
                ...
                {/* Contact Section */}
                <div style={{ background: 'linear-gradient(135deg, var(--brown), var(--brown-mid))', borderRadius: 16, padding: '24px', color: 'white', marginBottom: 20 }}>
                  <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, marginBottom: 6, color: 'var(--gold-light)' }}>Contact Us to Confirm</div>
                  <p style={{ fontSize: 14, opacity: 0.85, marginBottom: 20 }}>Reach out on WhatsApp or call us to confirm your order and arrange payment & delivery.</p>
                ...
                <button onClick={() => { setStep('menu'); setCustomerInfo({ name: '', phone: '', address: '', notes: '' }); setPlacedOrder(null); }} style={{
                  background: 'none',
