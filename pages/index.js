...
// Customer fills: items, name, phone, address, dietary needs, notes.
// Delivery fee, deposit, and invoice are admin-only (/admin).
...
export default function Home() {
    ...
    try {
      const res = await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          ...
          total: cartTotal,
          // deliveryFee and depositAmount are set by admin after review
        }),
      });
  ...
  return (
    <>
      ...
      {lightbox && (
        <div onClick={() => setLightbox(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      ...
      <div style={{ minHeight: '100vh', background: 'var(--cream)' }}>
        <header style={{ background: 'var(--brown)', padding: '0 24px', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}>
          <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 64 }}>
            ...
            {step === 'menu' && (
              <button onClick={() => cart.length && setStep('cart')} disabled={!cart.length} style={{
                background: cart.length ? 'var(--gold)' : 'rgba(255,255,255,0.15)',
                color: cart.length ? 'var(--brown)' : 'rgba(255,255,255,0.5)',
                border: 'none', borderRadius: 24, padding: '10px 20px', fontSize: 14, fontWeight: 700,
                cursor: cart.length ? 'pointer' : 'default', display: 'flex', alignItems: 'center', gap: 8, transition: 'all 0.3s',
              }}>
                🛒 Cart
                {cartCount > 0 && <span style={{ background: 'var(--brown)', color: 'white', borderRadius: '50%', width: 22, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 800 }}>{cartCount}</span>}
              </button>
            )}
          </div>
        </header>
        ...
        <main style={{ maxWidth: 1100, margin: '0 auto', padding: '0 24px 80px' }}>
          ...
          {step === 'menu' && (
            <div style={{ animation: 'fadeIn 0.4s ease' }}>
              <div style={{ textAlign: 'center', padding: '48px 0 32px' }}>
                <div style={{ display: 'inline-block', background: 'var(--gold)', color: 'var(--brown)', borderRadius: 20, padding: '4px 16px', fontSize: 12, fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 16 }}>Freshly Baked Daily</div>
                <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(32px, 5vw, 52px)', color: 'var(--brown)', lineHeight: 1.2, marginBottom: 12 }}>Blissful Bakes</h1>
                <p style={{ color: 'var(--text-muted)', fontSize: 16, maxWidth: 480, margin: '0 auto' }}>Handcrafted cakes & cupcakes made with love in Nairobi</p>
              </div>
                ...
                {filtered.map(item => {
                  ...
                  return (
                    <div key={item.id} style={{ background: 'white', borderRadius: 16, overflow: 'hidden', border: '1px solid var(--border)', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', transition: 'transform 0.2s, box-shadow 0.2s' }}
                      ...
                      <div onClick={() => setLightbox(item)} style={{ height: 200, background: 'linear-gradient(135deg, #f5e6d0, #e8c99a)', cursor: 'pointer', position: 'relative', overflow: 'hidden' }}>
                        <img src={item.image} alt={item.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => { e.target.style.display = 'none'; }} />
                        <div style={{ position: 'absolute', top: 10, right: 10, background: 'rgba(0,0,0,0.4)', color: 'white', borderRadius: 20, padding: '3px 10px', fontSize: 11, backdropFilter: 'blur(4px)' }}>{item.category}</div>
                      </div>
                      <div style={{ padding: '16px 16px 18px' }}>
                        ...
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
                          {item.sizes.map((s, idx) => (
                            <button key={idx} onClick={() => setSelectedSizes(prev => ({ ...prev, [item.id]: idx }))} style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, fontWeight: 700, color: 'var(--gold)' }}>Ksh {selectedSize.price.toLocaleString()}</span>
                          <button onClick={() => addToCart(item)} style={{ background: 'var(--brown)', color: 'white', border: 'none', borderRadius: 10, padding: '10px 18px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>+ Add</button>
                        </div>
                      </div>
                    </div>
                  );
          ...
          {step === 'cart' && (
            <div style={{ animation: 'fadeIn 0.4s ease', maxWidth: 600, margin: '0 auto', paddingTop: 40 }}>
              <button onClick={() => setStep('menu')} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: 14, marginBottom: 24 }}>← Back to Menu</button>
              ...
              {cart.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '48px 0', color: 'var(--text-muted)' }}>
                  ...
                  <button onClick={() => setStep('menu')} style={{ marginTop: 16, background: 'var(--brown)', color: 'white', border: 'none', borderRadius: 10, padding: '12px 24px', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>Browse Menu</button>
                </div>
              ) : (
                <>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 24 }}>
                    {cart.map(c => (
                      <div key={c.key} style={{ background: 'white', borderRadius: 14, padding: 16, border: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 14 }}>
                        <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, color: 'var(--gold)', fontSize: 16, minWidth: 90, textAlign: 'right' }}>Ksh {(c.size.price * c.qty).toLocaleString()}</div>
                  ...
                  <div style={{ background: 'white', borderRadius: 14, border: '1px solid var(--border)', padding: '16px 20px', marginBottom: 20 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--text-muted)', fontSize: 14, marginBottom: 8 }}>
                      <span>Subtotal ({cartCount} items)</span><span>Ksh {cartTotal.toLocaleString()}</span>
                    </div>
                  ...
                  <button onClick={() => setStep('details')} style={{ width: '100%', background: 'var(--brown)', color: 'white', border: 'none', borderRadius: 12, padding: 15, fontSize: 16, fontWeight: 700, cursor: 'pointer' }}>
                    Continue to Details →
                  </button>
                </>
              )}
          ...
          {step === 'details' && (
            <div style={{ animation: 'fadeIn 0.4s ease', maxWidth: 560, margin: '0 auto', paddingTop: 40 }}>
              <button onClick={() => setStep('cart')} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: 14, marginBottom: 24 }}>← Back to Cart</button>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                {[
                  ...
                  <div key={field.key}>
                    ...
                    <input type={field.type} placeholder={field.placeholder} value={customerInfo[field.key]}
                      onChange={e => setCustomerInfo(i => ({ ...i, [field.key]: e.target.value }))} style={inputStyle} />
                  </div>
                ))}
                ...
                <div>
                  ...
                  <textarea placeholder="e.g. nut-free, gluten-free, vegan..."
                    value={customerInfo.dietaryNeeds}
                ...
                <div>
                  ...
                  <textarea placeholder="Cake message, preferred delivery time, colours..."
              ...
              <div style={{ background: 'white', borderRadius: 14, border: '1px solid var(--border)', padding: '18px 20px', marginTop: 28, marginBottom: 20 }}>
                ...
                <div style={{ borderTop: '1px solid var(--border)', marginTop: 10, paddingTop: 10, display: 'flex', justifyContent: 'space-between', fontWeight: 700, fontSize: 18, color: 'var(--brown)' }}>
                  <span>Subtotal</span>
                  <span style={{ color: 'var(--gold)', fontFamily: "'Playfair Display', serif" }}>Ksh {cartTotal.toLocaleString()}</span>
                </div>
              </div>

              <button onClick={handlePlaceOrder} disabled={loading || !isFormValid} style={{
          ...
          {step === 'confirm' && placedOrder && (
            <div style={{ animation: 'fadeIn 0.5s ease', maxWidth: 520, margin: '0 auto', textAlign: 'center', paddingTop: 60 }}>
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: 15, marginBottom: 20 }}>
                We'll review your order and confirm it shortly via WhatsApp.
              </p>
              ...
              <div style={{ background: 'white', borderRadius: 16, border: '1px solid var(--border)', padding: '24px', textAlign: 'left', marginBottom: 28, boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
                <div style={{ fontWeight: 700, color: 'var(--brown)', marginBottom: 14, fontSize: 15, borderBottom: '1px solid var(--border)', paddingBottom: 12 }}>Order Summary</div>
                ...
                <div style={{ borderTop: '1px solid var(--border)', marginTop: 12, paddingTop: 12, display: 'flex', justifyContent: 'space-between', fontWeight: 700, fontSize: 16, color: 'var(--brown)' }}>
                  <span>Subtotal</span>
                ...
                <p style={{ marginTop: 12, fontSize: 13, color: 'var(--text-muted)', fontStyle: 'italic' }}>
                  Final total including delivery will be confirmed by Blissful Bakes.
                </p>
              ...
              <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 20 }}>
                <a href={`https://wa.me/${contact.whatsapp}?text=${encodeURIComponent(whatsappMsg)}`} target="_blank" rel="noopener noreferrer"
              ...
              <button onClick={() => { setStep('menu'); setCustomerInfo({ name: '', phone: '', address: '', notes: '', dietaryNeeds: '' }); setPlacedOrder(null); }}
